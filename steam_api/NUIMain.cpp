...
extern IDirect3DDevice9* g_nuiDevice;
...
void NUI_Init()
{
	...
	g_nuiImage = &g_nui.nuiImage;
	Image_Setup(&g_nui.nuiImage, g_nuiWidth, g_nuiHeight, 1, 0x1000003, D3DFMT_A8R8G8B8);
	...
	Dvar_SetCommand("name", Config_GetUsername());
	// make sure this doesn't get duplicated too much
	Dvar_SetCommand("steamid", va("%i", GetHardwareID()));
}
